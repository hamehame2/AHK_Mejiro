...
#UseHook True
; === Mejiro AHK build: PRACTICE-SPEC v21 ===
...
#Include *i mejiro_practice_logic_v21.ahkinc
...
if !IsSet(PRACTICE_EXACT_MAP)
    global PRACTICE_EXACT_MAP := Map()
...
Mejiro_Lookup(stroke) {
    ...
    ; 練習サイトを仕様とする最優先の完全一致表。
    ; 重複して複数の出力を持つストロークは表に入れず、下の規則処理へ渡す。
    global PRACTICE_EXACT_MAP
    if (PRACTICE_EXACT_MAP.Has(stroke))
        return Mejiro_FinalizeResult(PRACTICE_EXACT_MAP[stroke])
