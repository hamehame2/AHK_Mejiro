...
## メジロ式関連事項2
またzmk版メジロ式（qmk2.6.2合わせ）を公開しています。zmkキーボードだけで、ソフト不要で動きますのでそちらもどうぞ使用ください。

https://github.com/hamehame2/zmk-behavior-mejiro/tree/main

下記はploverを使える人用です。IMEを介するのが面倒な方、マシンパワーがある方、英語も速記にしたい方向けです。

https://github.com/hamehame2/zmk-behavior-plover64

## メジロ式　フロー
ploverが使える→NKROもしくは NKROは不要なQMK plover HIDを使用、もしくはNKRO不要なのは同じですが先のzmk ploverを使用ください。

ploverが使えない→qmkが使えるならqmkメジロ式、zmkならzmkメジロ式、どちらも使用できないならAHKメジロ式を検討ください（NKRO必要）。
